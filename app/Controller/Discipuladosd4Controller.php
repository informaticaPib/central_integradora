...
class Discipuladosd4Controller extends AppController
{
	public $uses = array(
		'Clientepib',
		'Euvim'
	...
	public function perfil()
	{
        ...
        $dadosVinculo = $this->MembroVinculo->find('first', array(
            ...
            'conditions' => 'MembroVinculo.nivel_discipulo = 4 AND MembroVinculo.ativo = 1 AND MembroVinculo.membro_id = '.$this->request->params['pass'][0].''
        ));
        ...
        $totalPresencasEuvimEstudosD4 = $this->Euvim->find('count', array(
	...
	public function vinculados()
	{
		...
		$totalDiscipulosComVinculo = $this->MembroVinculo->totalComVinculoD4();								
		...
		$this->paginate = array(
			...
			',	
			'conditions' => 'MembroVinculo.ativo = 1 AND MembroVinculo.nivel_discipulo = 4 AND Clientepib.ativo = 1 AND Clientepib.status_id = 5'.$where,			
			'joins' => array(
				array(
					'type' => 'inner',
					'table' => 'membros',
	...
	public function naovinculados()
	{
        ...
        $listaTimes = $this->User->listaTimes();
		$listaRedes = $this->Rede->getListAllRedes();		
		$totalDiscipulosSemVinculo = $this->Clientepib->totalSemVinculoD4();								
		...
		$this->paginate = array(
			...
			'conditions' => 'Clientepib.status_discipulado = 4 AND Clientepib.is_discipulador = 1 AND Clientepib.ativo = 1 AND Clientepib.status_id = 5 AND Clientepib.id NOT IN(SELECT mv.membro_id FROM membros_vinculos AS mv WHERE mv.ativo = 1 AND mv.nivel_discipulo = 4)'.$where,
			'limit' => 20,
			'order' => 'Clientepib.nome'
		);
		
		$listaNaoVinculadosD4 = $this->paginate('Clientepib');				
		...
		$this->set(compact('totalDiscipulosSemVinculo', 'listaDiscipuladores', 'listaNaoVinculadosD4', 'listaRedes', 'listaDiscipuladores', 'listaTimes', 'listaOrigens'));
	}
	...
	public function addfrequencia()
	{
		...
		$this->redirect(array('controller' => 'discipuladosd4', 'action' => 'perfil', $this->request->data['Euvim']['id_membro']));
	}
	...
	public function vinculardiscipulador()
	{
		...
		if ($this->request->is('post')) {
			...
			try {
				...
				$this->MembroVinculo->set('obs', 'Vínculo D4 criado via central integradora');
				...
				if(empty($idrede)){
					...
					$this->redirect(array('controller' => 'discipuladosd4', 'action' => 'naovinculados'));
				}
	...
	public function enviard4()
	{
		...
		if ($this->request->is('post')) {
			...
			if ($sucesso == 0) {
				$this->Session->setFlash('Erro ao finalizar discipulado, tente novamente', 'error');
				$this->redirect(array('controller' => 'discipuladosd4', 'action' => 'perfil', $id));
			}
			else {
				$this->Session->setFlash('Discipulad finalizado!', 'success');
				$this->redirect(array('controller' => 'discipuladosd4', 'action' => 'vinculados'));
			}
		}
	}
