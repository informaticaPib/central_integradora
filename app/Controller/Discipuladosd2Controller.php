...
class Discipuladosd2Controller extends AppController
{
	public $uses = array(
		'Clientepib',
		'Euvim'
	...
	public function perfil()
	{
		...
		if (empty($verificaMembro)) {
			...
			$this->redirect(array('controller' => 'discipuladosd2', 'action' => 'vinculados'));
		} else {			
			...
			$dadosVinculo = $this->MembroVinculo->find('first', array(
				...
				'conditions' => 'MembroVinculo.ativo = 1 AND MembroVinculo.membro_id = '.$this->request->params['pass'][0].''
			));
			...
			if (($dadosMembro['MembroVinculo']['discipulador_id'] != $this->Session->read('Auth.User.id')) && ($permissaoOperador == 0)) {
				...
				$this->redirect(array('controller' => 'discipuladoresd2', 'action' => 'index'));
			}
			
			$euvim = $this->Euvim->find('all', array(
				'fields' => 'Euvim.*, Turma.nome, Linha.nome, Aula.nome',
				'conditions' => 'Euvim.id_membro = ' . $idMembro . '',
				'joins' => array(
					array(
						'type' => 'INNER',
						'table' => 'turmas',
			...
			$totalPresencasEuvimEstudosD2 = $this->Euvim->find('count', array(
				'conditions' => 'Euvim.id_membro = ' . $idMembro . ' AND Euvim.id_turma IN(7,25)',
				'recursive' => -1
			));
			
			$totalPresencasEuvim = $totalPresencasEuvimEstudosD2;

			//Total Frequencia Dons e Espiritualidade
            $totalDonsEspiritualidade = $this->Euvim->totalFrequencia($idMembro,7);
			//Total Frequencia Eu, um Discipulador
            $totalEuDiscipulador = $this->Euvim->totalFrequencia($idMembro,25);
			//Total Frequencia Liderando uma célula saudavel
            $totalLiderandoCelula = $this->Euvim->totalFrequencia($idMembro,6);
			...
			$listaTodosDadosDiscipuladores = $this->Discipulador->find("all", array(
				...
				"conditions" => "Discipulador.status = 1 AND Discipulador.nivel_discipulador = 3",
				"joins" => array(
					array(
						"type" => "LEFT",
			...
			$this->set(compact('totalDonsEspiritualidade','totalLiderandoCelula','totalEuDiscipulador','statusDiscipulado','listaTimes', 'dadosDiscipulador', 'listaDiscipuladores', 'listaTodosDadosDiscipuladores', 'dadosMembro', 'euvim', 'totalPresencasEuvim', 'totalPresencasEuvimDiscipulado', 'totalPresencasEuvimIgreja', 'historicoAcompanhamentoDiscipulo', 'dadosCelulaApi'));
		}
	...
	public function vinculados()
	{
		...
		$totalDiscipulosComVinculo = $this->MembroVinculo->totalComVinculoD2();								
		...
		if ($this->request->is('get')) {
			...
			if ($this->params['url']['id_discipulador']) {
				...
				$where .= ' AND MembroVinculo.discipulador_id LIKE "%' . $discipulador . '%"';
			}
		...
		$this->paginate = array(
			...
			',	
			'conditions' => 'MembroVinculo.ativo = 1 AND MembroVinculo.nivel_discipulo = 2 AND Clientepib.ativo = 1 AND Clientepib.status_id = 5'.$where,			
			'joins' => array(
				array(
					'type' => 'inner',
					'table' => 'membros',
	...
	public function naovinculados()
	{
        ...
        $listaTimes = $this->User->listaTimes();
		$listaRedes = $this->Rede->getListAllRedes();		
		$totalDiscipulosSemVinculo = $this->Clientepib->totalSemVinculoD2();								
		...
		$this->paginate = array(
			...
			'conditions' => 'Clientepib.ativo = 1 AND Clientepib.status_id = 5 AND Clientepib.status_discipulado = 2 AND Clientepib.id NOT IN(SELECT mv.membro_id FROM membros_vinculos AS mv WHERE mv.ativo = 1 AND mv.nivel_discipulo = 2)' . $where,
			'limit' => 20,
			'order' => 'Clientepib.nome'
		);
		
		$listaNaoVinculadosD2 = $this->paginate('Clientepib');				
		...
		$this->set(compact('totalDiscipulosSemVinculo', 'listaDiscipuladores', 'listaNaoVinculadosD2', 'listaRedes', 'listaDiscipuladores', 'listaTimes', 'listaOrigens'));
	}
	...
	public function addfrequencia()
	{
		...
		//Total Frequencia Eu, um Discipulador
		$totalEuDiscipulador = $this->Euvim->totalFrequencia($this->request->data['Euvim']['id_membro'],25);
		if($totalEuDiscipulador >= 4){
			$this->Clientepib->updateAll(
				array(
					'status_discipulado' => 3,
					'is_discipulador' => 1,
				), array(
			...
			//Terminou o curso para se tornar discipulador, adiciono ele como um discipulador aguardando aprovação
			$this->Discipulador->create();
			$this->Discipulador->set('id_user', 0);
			$this->Discipulador->set('id_membro', $this->request->data['Euvim']['id_membro']);
			$this->Discipulador->set('nome', $this->request->data['Euvim']['nome_membro']);
			$this->Discipulador->set('cpf', $this->request->data['Euvim']['cpf_membro']);
			$this->Discipulador->set('horas_semanais', 1);
			$this->Discipulador->set('status_disponibilidade', 1);
			$this->Discipulador->set('disponib_casal', 0);
			$this->Discipulador->set('status', 0);
			$this->Discipulador->set('id_faixa', 0);
			$this->Discipulador->set('is_liberado', 0);
			$this->Discipulador->set('total_vinculo', 0);
			$this->Discipulador->set('nivel_discipulador', 3);
			$this->Discipulador->save();

			//Terminou o vinculo com o discipulador atual			
			$this->MembroVinculo->updateAll(
				array(
					'data_desvinculo' => '"'.$hojeCompleto.'"',
					'obs' => 'Processo discipulado finalizado',
					'ativo' => 0
				), array(
					'id_membro' => $this->request->data['Euvim']['id_membro'],
					'nivel_discipulo' => 2
				)
			);

			//Enviar email do servir
			$dadosTurma = $this->Euvimturma->findById(140);					
			...
			$nomeAluno = $this->request->data['Euvim']['nome_membro'];
			$emailAluno = $this->request->data['Euvim']['email_membro'];			
			$dadosHTML = array(
				'data' => $dataBrasileiro,
				'hora' => $hora,
				'link' => $linkReuniao
		...
		$this->redirect(array('controller' => 'discipuladosd2', 'action' => 'perfil', $this->request->data['Euvim']['id_membro']));
	}
	...
	public function vinculardiscipulador()
	{
		...
		if ($this->request->is('post')) {
			...
			try {
				...
				$this->MembroVinculo->set('obs', 'Vínculo D2 criado via central integradora');
				...
				if(empty($idrede)){
					...
					$this->redirect(array('controller' => 'discipuladosd2', 'action' => 'naovinculados'));
				}
	...
	public function enviard3()
	{
		...
		if ($this->request->is('post')) {
			...
			if(empty($observacao) || $observacao == ""){
				$observacao = "Discipulado D2 finalizado direto via central integradora.";
			}
			$hojeCompleto = date("Y-m-d H:i:s");
			...
			$dadosVinculo = $this->MembroVinculo->find('first', array(
				'fields' => 'MembroVinculo.*',
				'conditions' => 'MembroVinculo.ativo = 1 AND MembroVinculo.nivel_discipulo = 2'
			));
			...
			if ($sucesso == 0) {
				...
				$this->redirect(array('controller' => 'discipuladosd2', 'action' => 'perfil', $id));
			}
			else {
