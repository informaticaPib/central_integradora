...
<div class="row">
    <div class="col-md-12">
        <h2>Discípulos vinculados (<?php echo $totalDiscipulosComVinculo; ?>) para formação de discipuladores de líderes (D3)</h2>        
    </div>
</div>
<div class="row">
	<?php
		echo $this->Form->create('Discipulador', array(
			'url' => array('controller' => 'discipuladosd3', 'action' => 'vinculados'),
			'type' => 'get',
			'class' => 'form-inline'
		));
	?>
        ...
        foreach( $discipulosVinculados as $lista ){
            ...
            elseif($lista['Clientepib']['status_id'] == 30){
                ...
                if($ultimoHistorico >= $dataAtual ){
					echo '<td>';						
						echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fas fa-user fa-2x', 'title'=>'Perfil completo' ,'alt'=>'Perfil completo', 'style'=>'color:black')).'',array('controller' => 'discipuladosd3', 'action' => 'perfil', $lista['Clientepib']['id']), array('escape' => false));
						echo "<i class='fas fa-comments fa-2x' style='color:green' title='Acompanhamento' alt='Acompanhamento' data-toggle='modal' data-target='#acompanhamentodiscipulo' id='idBtnAcompanhamento' onclick='acompanhamentos(\"{$lista['Discipulador']['id']}\",\"{$lista[0]['nomeDiscipulador']}\",\"{$lista['Clientepib']['id']}\",\"{$lista['Clientepib']['nome']}\")';> </i>";
					echo '</td>';
