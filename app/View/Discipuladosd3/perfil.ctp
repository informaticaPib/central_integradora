            ...
            <div class="modal-body">
				...
				<?php
					echo $this->Form->create('Euvim', array(
						'url' => array('controller' => 'discipuladosd3', 'action' => 'addfrequencia'),
            ...
            <div class="modal-body">
				...
				<?php
					echo $this->Form->create('discipulado', array(
						'url' => array('controller' => 'discipuladosd3', 'action' => 'definirtime'),
						'class' => 'form-horizontal'
					));
					echo $this->Form->input("id_membro", array("type" => "hidden", "value"=>$idPib));
    ...
    <div class="modal-dialog" role="document">
		<?php
			echo $this->Form->create('Historicoacompanhamentodiscipulo', array(
				'url' => array('controller' => 'discipuladosd3', 'action' => 'registraracompanhamento'),
				'class' => 'form-horizontal'
			));
...
<div class="modal fade" id="modalFinalizarDiscipulado" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">Finalizar Discipulado III (Avançado) - Formação de Líderes</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
            <p><b>IMPORTANTE:</b> Realizado está ação, você estará passando a pessoa automaticamente para o próximo nível, onde todas as presenças nos cursos vão ser registradas.</p>
				<?php
					echo $this->Form->create('discipulado', array(
						'url' => array('controller' => 'discipuladosd3', 'action' => 'enviard4'),
						'class' => 'form-horizontal'
					));
					echo $this->Form->input("id_membro", array("type" => "hidden", "value"=>$idPib));
					echo $this->Form->input("nome_membro", array("type" => "hidden", "value"=>$nome));
    ...
    <div class="modal-dialog modal-lg" role="document">
		<?php
			echo $this->Form->create('Discipulador', array(
				'url' => array('controller' => 'discipuladosd3', 'action' => 'vinculardiscipulador'),
				'class' => 'form-horizontal'
			));
		?>
...
<script type="text/javascript">
    ...
    function cancelarVinculo(idPib){
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type:"POST",
                    url:"<?php echo $this->Html->url(array("controller" => "discipuladosd3","action"=> "cancelarvinculo"), true); ?>",
                    data: 'idpib='+idPib,
                    dataType: 'json',
                    async: true,
                    success:function(data){
    ...
    function buscarAulas(modulo){
        $.ajax({
            type:"POST",
            url:"<?php echo $this->Html->url(array("controller" => "discipuladosd3","action"=> "buscaaulas"), true); ?>",
            data: 'modulo='+modulo,
            dataType: 'json',
            async: true,
            success:function(data){
  ...
  /* $("#idEnviaLiberacao").click(function () {
    ...
    function buscarEuVim(idModulo){
        $.ajax({
            type:"POST",            
            url:"<?php echo $this->Html->url(array("controller" => "discipuladosd3","action"=> "euvim"), true); ?>",
