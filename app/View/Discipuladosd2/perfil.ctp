    ...
    <div class="col-md-12">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade show" id="euvim" role="tabpanel" aria-labelledby="euvim-tab">
                <ul class="nav nav-tabs nav-pills">
                    <li class="nav-item">
                        <a class="nav-link" id="euvim-tab-discipulado" data-toggle="tab" href="#euvimresult" role="tab" aria-selected="false" onclick="buscarEuVim(7)">Dons e Espiritualidade (<span id="totalEuVimDonsEspiritualidade"></span>)</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" id="euvim-tab-serigreja" data-toggle="tab" href="#euvimresult" role="tab" aria-selected="false" onclick="buscarEuVim(25)">Eu, um discipulador (<span id="totalEuVimEuUmDiscipulador"></span>)</a>
                    </li>
                </ul>
                <div class="tab-content">
...
<div class="row">
    <div class="col-md-3">
        ...
        if($statusDiscipulado == 0){
            ...
            if($totalEuDiscipulador >= 4 && $totalDonsEspiritualidade >= 6){
                echo '<p>Processo finalizado, todas as aulas desse nível já foram realizadas.</p>';
            }
        }        
        ...
        echo '<button class="btn btn-default btnAcaoPerfilDiscipulo" data-toggle="modal" data-target="#modalFinalizarDiscipuladoNivel2">FINALIZAR DISCIPULADO DIRETO</button>';
        ?>
    </div>
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="modal-body">
				...
				<?php
					echo $this->Form->create('Euvim', array(
						'url' => array('controller' => 'discipuladosd2', 'action' => 'addfrequencia'),
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" required="required" name="data[Euvim][id_turma]" onchange="buscarAulas(this.value)">
                        ...
                        <?php 
                        if($totalEuDiscipulador >= 4 && $totalDonsEspiritualidade >= 6){
                            echo '<option value="">TODOS OS MÓDULOS JÁ FORAM FINALIZADOS</option>';
                        }else{
                            echo '<option value="7">DONS E ESPIRITUALIDADE</option>';
                            echo '<option value="25">EU, UM DISCIPULADOR</option>';
                        }                        
                        ?>
                    </select>
            ...
            <div class="modal-body">
				...
				<?php
					echo $this->Form->create('discipulado', array(
						'url' => array('controller' => 'discipuladosd2', 'action' => 'definirtime'),
						'class' => 'form-horizontal'
					));
					echo $this->Form->input("id_membro", array("type" => "hidden", "value"=>$idPib));
    ...
    <div class="modal-dialog" role="document">
		<?php
			echo $this->Form->create('Historicoacompanhamentodiscipulo', array(
				'url' => array('controller' => 'discipuladosd2', 'action' => 'registraracompanhamento'),
				'class' => 'form-horizontal'
			));
...
<!-- Modal FINALIZAR DISCIPULADO D2 -->
<div class="modal fade" id="modalFinalizarDiscipuladoNivel2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">Finalizar Discipulado II (Intermediário) - Formação de Discipuladores</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
                <p><b>IMPORTANTE:</b> Realizado está ação, você estará passando a pessoa automaticamente para o próximo nível, onde todas as presenças nos cursos vão ser registradas.</p>
				<?php
					echo $this->Form->create('discipulado', array(
						'url' => array('controller' => 'discipuladosd2', 'action' => 'enviard3'),
						'class' => 'form-horizontal'
					));
					echo $this->Form->input("id_membro", array("type" => "hidden", "value"=>$idPib));
					echo $this->Form->input("nome_membro", array("type" => "hidden", "value"=>$nome));
...
<!-- Modal FINALIZAR DISCIPULADO D2 Nivel 2 -->
<div class="modal fade" id="modalFinalizarDiscipuladoNivel2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">Finalizar discipulado(d2) formação de líderes</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
                <p><b>Está pessoa será enviada para o rito de passagem, onde a mesma vai ter que comparecer a um encontro presencial. Todos os detalhes serão enviados por email <br>Próximo nível: FORMAÇÃO DE SUPERVISORES(D3)</b></p>
				<?php
					echo $this->Form->create('discipulado', array(
						'url' => array('controller' => 'discipuladosd2', 'action' => 'enviard2nivel2'),
						'class' => 'form-horizontal'
					));
					echo $this->Form->input("id_membro", array("type" => "hidden", "value"=>$idPib));
					echo $this->Form->input("nome_membro", array("type" => "hidden", "value"=>$nome));
    ...
    <div class="modal-dialog modal-lg" role="document">
		<?php
			echo $this->Form->create('Discipulador', array(
				'url' => array('controller' => 'discipuladosd2', 'action' => 'vinculardiscipulador'),
				'class' => 'form-horizontal'
			));
		?>
...
<script type="text/javascript">
    ...
    function cancelarVinculo(idPib){
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type:"POST",
                    url:"<?php echo $this->Html->url(array("controller" => "discipuladosd2","action"=> "cancelarvinculo"), true); ?>",
                    data: 'idpib='+idPib,
                    dataType: 'json',
                    async: true,
                    success:function(data){
    ...
    function buscarAulas(modulo){
        $.ajax({
            type:"POST",
            url:"<?php echo $this->Html->url(array("controller" => "discipuladosd2","action"=> "buscaaulas"), true); ?>",
            data: 'modulo='+modulo,
            dataType: 'json',
            async: true,
            success:function(data){
  ...
  /* $("#idEnviaLiberacao").click(function () {
    ...
    function buscarEuVim(idModulo){
        $.ajax({
            type:"POST",            
            url:"<?php echo $this->Html->url(array("controller" => "discipuladosd2","action"=> "euvim"), true); ?>",
            ...
            success:function(data){
                ...
                if(idModulo == 7){
                    $("#totalEuVimDonsEspiritualidade").html(totalRespondido);
                }else{
                    $("#totalEuVimEuUmDiscipulador").html(totalRespondido);
                }
            }
        });
    }
