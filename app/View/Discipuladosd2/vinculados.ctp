...
<div class="row">
    <div class="col-md-12">
        <h2>Discípulos vinculados (<?php echo $totalDiscipulosComVinculo; ?>) para formação de líderes e discipuladores (D2)</h2>        
    </div>
</div>
<div class="row">
	<?php
		echo $this->Form->create('Discipulador', array(
			'url' => array('controller' => 'discipuladosd2', 'action' => 'vinculados'),
			'type' => 'get',
			'class' => 'form-inline'
		));
	?>
        ...
        foreach( $discipulosVinculados as $lista ){
            ...
            elseif($lista['Clientepib']['status_id'] == 30){
                ...
                if($ultimoHistorico >= $dataAtual ){
					echo '<td>';						
						echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fas fa-user fa-2x', 'title'=>'Perfil completo' ,'alt'=>'Perfil completo', 'style'=>'color:black')).'',array('controller' => 'discipuladosd2', 'action' => 'perfil', $lista['Clientepib']['id']), array('escape' => false));
