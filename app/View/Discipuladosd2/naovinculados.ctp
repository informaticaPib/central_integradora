...
<div class="row"> 
    <div class="col-md-12">
        <h2>Discípulos não vinculados(<?php echo $totalDiscipulosSemVinculo; ?>) para formação de líderes e discipuladores (D2)</h2>
    </div>
</div>
<div class="row">
	<?php
		echo $this->Form->create('Discipulador', array(
			'url' => array('controller' => 'discipuladosd2', 'action' => 'naovinculados'),
			'type' => 'get',
			'class' => 'form-inline'
		));
	?>
...
<?php
	...
	echo "</thead>";
	echo "<tbody>";	
	foreach( $listaNaoVinculadosD2 as $lista ){		
		...
		echo '<td>';
			...
			echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fas fa-user', 'title'=>'Perfil completo' ,'alt'=>'Perfil completo', 'style'=>'color:black')).'',array('controller' => 'discipuladosd2', 'action' => 'perfil', $lista['Clientepib']['id']), array('escape' => false));
		echo '</td>';
